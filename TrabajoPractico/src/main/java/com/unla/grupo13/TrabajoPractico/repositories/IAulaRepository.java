...
public interface IAulaRepository extends JpaRepository<Aula,Integer>{
	...
	public abstract Tradicional findByIdTradWithEspacios(char turno,boolean libre, int id);

	@Query("SELECT l FROM Laboratorio l JOIN FETCH l.espacios e WHERE e.libre=(:libre) and e.turno=(:turno) and l.id=(:id)")
	public abstract Laboratorio findByIdLabWithEspacios(char turno, boolean libre, int id);
