...
public interface IEspacioRepository extends JpaRepository<Espacio, Serializable> {
	...
	public abstract Espacio findByLibreFechaAula(@Param("aula") Aula aula, @Param("turno") char turno,
	...
	public abstract List<Laboratorio> findByLaboratorioLibre(@Param("libre") boolean libre, @Param("turno") char turno,
			@Param("cantSillas") int cantSillas);
	...
	@Query("SELECT e FROM Espacio e JOIN FETCH e.notaPedido n WHERE n.codCurso=(:codCurso)")
	public abstract List<Espacio> findByCodCurso(@Param("codCurso") String codCurso);

	@Query("SELECT e FROM Espacio e JOIN FETCH e.notaPedido n WHERE n != null")
	public abstract List<Espacio> findAllAsignados();
